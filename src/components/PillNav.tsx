...
const PillNav: React.FC<PillNavProps> = ({
  ...
  useEffect(() => {
    const layout = () => {
      circleRefs.current.forEach((circle, index) => {
        ...
        if (white) {
          gsap.set(white, { y: h + 8, opacity: 0 });
  ...
  const handleLeave = (i: number) => {
    const tl = tlRefs.current[i];
    if (!tl) return;
    const pill = circleRefs.current[i]?.parentElement as HTMLElement;
    if (pill && pill.classList.contains('is-active')) return; // Don't hide active pill
    activeTweenRefs.current[i]?.kill();
