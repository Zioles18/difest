...
export function ActionItems() {
    ...
    const toggleDone = (id: number) => {
        setItems((prev) => prev.map((item) => item.id === id ? { ...item, done: !item.done, icon: !item.done ? CheckCircle2 : (item.status === 'urgent' ? AlertCircle : Clock), color: !item.done ? 'text-emerald-500' : (item.status === 'urgent' ? 'text-rose-500' : 'text-amber-500') } : item));
    };
    return (<SpotlightCard className="h-full">
      <div className="flex justify-between items-center mb-6 w-full z-10">
        ...
        <button onClick={() => console.log("View All Action Items clicked")} className="text-xs font-medium text-slate-900 dark:text-slate-100 hover:text-slate-500 dark:hover:text-slate-400 transition-colors">
          View All
        </button>
      </div>
      ...
      <div className="space-y-3 z-10">
        {items.map((item, index) => {
            ...
            return (<div key={item.id} onClick={() => toggleDone(item.id)} className={`flex items-center gap-4 p-3.5 hover:bg-slate-50 dark:hover:bg-slate-700/30 hover:shadow-[0_2px_10px_-4px_rgba(15,23,42,0.05)] dark:hover:shadow-none rounded-2xl transition-all cursor-pointer group border active:scale-[0.99] ${item.done ? "opacity-60 border-slate-100 dark:border-slate-700/50 bg-slate-50 dark:bg-slate-800/50 grayscale pt-3" : "border-transparent hover:border-slate-100 dark:hover:border-slate-700/50"}`}>
              <div className={`mt-0.5 ${item.color} bg-white dark:bg-slate-800 shadow-sm ring-1 ring-slate-100 dark:ring-slate-700/50 group-hover:ring-transparent group-hover:bg-opacity-10 p-2.5 rounded-xl transition-all duration-300`}>
