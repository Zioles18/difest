...
import { LayoutDashboard, Users, ShoppingBag, Settings, PieChart, X, LogOut, ChevronRight, TrendingUp, Award } from "../Icons";
...
function SidebarContent({ setIsOpen }: {
    ...
    return (<div className="w-64 h-full flex flex-col bg-white dark:bg-slate-900 border-r border-slate-100 dark:border-slate-800/50 transition-all duration-300">
      ...
      <div className="h-20 flex items-center justify-between px-6 border-b border-slate-100 dark:border-slate-800/50 shrink-0">
        ...
        <button className="lg:hidden p-2 text-slate-400 dark:text-slate-500 hover:text-slate-700 dark:hover:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700/50 rounded-lg transition-colors" onClick={() => setIsOpen(false)}>
          <X className="w-5 h-5"/>
        </button>
      </div>
      ...
      <nav className="flex-1 py-5 px-3 overflow-y-auto">
        ...
        <div className="space-y-0.5">
          {menuItems.map((item) => {
            ...
            return (<NavLink key={item.to} to={item.to} end={item.to === "/"} onClick={() => setIsOpen(false)} className={({ isActive }) => `group flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all duration-200 ${isActive
        ...
        <div className="mt-8 px-3">
           <div className="p-4 bg-slate-50 dark:bg-slate-800/50 rounded-2xl border border-slate-100 dark:border-slate-700/50">
              ...
              <div className="h-1.5 w-full bg-slate-200 dark:bg-slate-700 rounded-full overflow-hidden">
                 <div style={{ width: `${Math.min(100, progress)}%`, transition: 'width 1s ease-in-out' }} className="h-full bg-indigo-600 rounded-full"/>
              </div>
      ...
      <div className="shrink-0 border-t border-slate-100 dark:border-slate-700/50 p-3">
        {isPremium && (<div className="mb-2 px-3 py-1.5 bg-indigo-50 dark:bg-indigo-500/10 rounded-lg flex items-center gap-2">
        ...
        <NavLink to="/profile" onClick={() => setIsOpen(false)} className={({ isActive }) => `group flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all duration-200 ${isActive
            ? "bg-indigo-50 dark:bg-indigo-500/10 ring-1 ring-indigo-200 dark:ring-indigo-800"
            : "hover:bg-slate-50 dark:hover:bg-slate-700/50"}`}>
          <div className="relative shrink-0">
            <img src={profile.avatar} alt={profile.name} className="w-9 h-9 rounded-xl border-2 border-white dark:border-slate-700/50 shadow-sm object-cover"/>
        ...
        <button onClick={() => { auth.logout(); navigate("/login"); }} className="mt-1 w-full flex items-center gap-3 px-3 py-2 rounded-xl text-slate-400 dark:text-slate-500 hover:text-rose-500 dark:hover:text-rose-400 hover:bg-rose-50 dark:hover:bg-rose-500/10 transition-all text-sm font-bold group text-left">
...
export function Sidebar({ isOpen, setIsOpen }: SidebarProps) {
    return (<>
      ...
      {isOpen && (<>
          <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-[500] lg:hidden transition-opacity duration-300" onClick={() => setIsOpen(false)}/>
          <div className="fixed top-0 left-0 h-full z-[600] lg:hidden shadow-2xl transition-transform duration-300 translate-x-0">
            <SidebarContent setIsOpen={setIsOpen}/>
          </div>
        </>)}
    </>);
