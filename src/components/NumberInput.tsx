    ...
    <div className={`space-y-3 ${className}`}>
      {label && (
        <div className="flex items-center justify-between pl-1">
          ...
          {showMultiplier && (
            <div className="flex bg-slate-100 dark:bg-white/5 p-1 rounded-xl gap-1 border border-transparent dark:border-white/5 dark:border-t-white/20 backdrop-blur-md transition-all">
              {([1, 10, 100] as const).map((m) => (
                <button
                  ...
                  className={`px-3 py-1 text-[11px] font-black rounded-xl transition-all ${
                    multiplier === m
                      ? "bg-white dark:bg-white/10 text-indigo-600 dark:text-indigo-400 shadow-sm dark:shadow-xl dark:shadow-black/20 border-t border-transparent dark:border-t-white/20"
