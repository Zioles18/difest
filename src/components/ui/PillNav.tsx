...
const PillNav: React.FC<PillNavProps> = ({ logo, logoAlt = 'Logo', items, activeHref, className = '', baseColor = '#fff', pillColor = '#120F17', hoveredPillTextColor = '#120F17', pillTextColor, onMobileMenuClick, }) => {
    ...
    return (<div className="pill-nav-container animate-fade-in-up z-50 relative">
      <nav className={`pill-nav ${className}`} aria-label="Primary" style={cssVars}>
        {isRouterLink(items?.[0]?.href) ? (<Link className="pill-logo hover:scale-110 hover:rotate-180 transition-all duration-300" to={items[0].href} aria-label="Home">
            <img src={logo} alt={logoAlt}/>
          </Link>) : (<a className="pill-logo hover:scale-110 hover:rotate-180 transition-all duration-300" href={items?.[0]?.href || '#'} aria-label="Home">
            <img src={logo} alt={logoAlt}/>
          </a>)}
        ...
        <div className="pill-nav-items desktop-only">
          <ul className="pill-list">
            ...
            return (<li key={item.href} className="flex" onMouseEnter={() => setHoveredIndex(i)} onMouseLeave={() => setHoveredIndex(null)}>
                  {isRouterLink(item.href) ? (<Link to={item.href} className={`pill flex items-center justify-center w-full h-full ${isActive ? 'is-active' : ''}`} aria-label={item.ariaLabel || item.label}>
                      {content}
                    </Link>) : (<a href={item.href} className={`pill flex items-center justify-center w-full h-full ${isActive ? 'is-active' : ''}`} aria-label={item.ariaLabel || item.label}>
                      {content}
                    </a>)}
                </li>);
        })}
      ...
      <div className={`mobile-menu-popover mobile-only transition-all duration-300 origin-top ${isMobileMenuOpen ? 'opacity-100 scale-y-100 visible' : 'opacity-0 scale-y-0 invisible'}`} style={cssVars}>
        <ul className="mobile-menu-list">
          {items.map(item => (<li key={item.href}>
              {isRouterLink(item.href) ? (<Link to={item.href} className={`mobile-menu-link ${activeHref === item.href ? 'text-[var(--pill-bg)] font-bold' : ''}`} onClick={() => setIsMobileMenuOpen(false)}>
                  {item.label}
                </Link>) : (<a href={item.href} className={`mobile-menu-link ${activeHref === item.href ? 'text-[var(--pill-bg)] font-bold' : ''}`} onClick={() => setIsMobileMenuOpen(false)}>
                  {item.label}
                </a>)}
            </li>))}
        </ul>
