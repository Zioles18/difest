  ...
  return (
    <div className={`space-y-3 ${className}`}>
      ...
      <div className="relative group flex items-center">
        {Icon && (
          <Icon className="absolute left-5 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-600 dark:text-slate-300 pointer-events-none" />
        )}
        <input
          id={id}
          name={name}
          ...
          className={`w-full bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-3xl h-[68px] text-lg font-bold text-slate-900 dark:text-white focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-400 outline-none transition-all placeholder:text-slate-300 dark:placeholder:text-slate-600 ${
            showControls ? "pr-36" : "pr-8"
          } ${
            Icon ? "pl-14" : "pl-8"
          }`}
        />
        {showControls && (
          <div className="absolute right-2.5 flex items-center gap-2">
            <button
              ...
              className="w-12 h-12 flex items-center justify-center bg-white dark:bg-slate-700 border border-slate-200 dark:border-slate-600 rounded-2xl text-slate-600 dark:text-white hover:text-indigo-500 hover:border-indigo-200 dark:hover:bg-slate-600 transition-all active:scale-90"
              aria-label="Decrease"
            >
