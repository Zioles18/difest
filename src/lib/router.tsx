...
export function RouterProvider({ children }: { children: React.ReactNode }) {
  ...
  return (
    <RouterCtx.Provider value={{ route, navigate, outletContext, setOutletContext }}>
      {children}
    </RouterCtx.Provider>
  );
}
...
export function Link({ to, children, className, onClick, ...rest }: { to: string; children: React.ReactNode; className?: string; onClick?: () => void; [key: string]: any }) {
  const { navigate } = useContext(RouterCtx);
  return (
    <button type="button" className={className} onClick={() => { navigate(to as Route); onClick?.(); }} {...rest}>
      {children}
    </button>
  );
}

export function NavLink({ to, children, className, onClick, end, ...rest }: { to: string; children: React.ReactNode | ((props: { isActive: boolean }) => React.ReactNode); className?: string | ((props: { isActive: boolean }) => string); onClick?: () => void; end?: boolean; [key: string]: any }) {
  ...
  return (
    <button type="button" className={cls} onClick={() => { navigate(to as Route); onClick?.(); }} {...rest}>
      {childContent}
    </button>
  );
}
