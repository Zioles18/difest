import { BarChart3, TrendingUp, Zap, X, ArrowRight, PieChart as PieIcon } from "../../components/Icons";
    ...
    return (<div className="space-y-6 sm:space-y-8 animate-fade-in transition-all duration-300">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 transition-all duration-300">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <div className="relative inline-flex items-center">
              <RotatingText texts={["Analytics", "Insights", "Growth", "Trends"]} mainClassName="text-xl sm:text-2xl font-bold text-slate-500 dark:text-slate-400 relative z-10" staggerDuration={0.02} rotationInterval={3500} splitBy="characters"/>
            </div>
          </div>
        ...
        <div className="flex items-center gap-3">
          <button onClick={() => setShowReport(true)} className="flex items-center gap-2 px-4 sm:px-6 py-2 sm:py-2.5 bg-indigo-600 dark:bg-indigo-500 text-white font-semibold rounded-xl hover:bg-indigo-700 transition-all shadow-lg shadow-indigo-500/20 dark:shadow-indigo-500/10 text-xs sm:text-sm">
        ...
        <div className="space-y-4 sm:space-y-8">
          <SpotlightCard className="p-6 sm:p-8">
             ...
             <div className="h-[180px] sm:h-[200px] w-full flex items-center justify-center">
                <div className="w-40 h-40 rounded-full transition-all duration-500 shadow-inner" style={{ background: generateConicGradient(deviceData) }}/>
             </div>
             <div className="mt-8 space-y-4">
                {deviceData.map((d: any) => (<div key={d.name} className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
      ...
      {showReport && (<div className="fixed inset-0 z-[200] flex items-center justify-center p-4 transition-all duration-300">
          <div onClick={() => setShowReport(false)} className="absolute inset-0 bg-slate-900/60"/>
          <div className="relative w-full max-w-2xl max-h-[85vh] bg-white dark:bg-slate-900 rounded-[2rem] shadow-2xl flex flex-col overflow-hidden transition-all duration-300">
            ...
            ].map((idx) => (<div key={idx.label} className={`${idx.bg} p-4 rounded-2xl text-center`}>
            ...
            ].map((rec, i) => (<div key={i} className="p-4 bg-slate-50 dark:bg-slate-800 rounded-xl border border-slate-100 dark:border-slate-700/50 flex items-start gap-4">
             ...
             <div className="p-5 sm:p-6 bg-slate-50 dark:bg-slate-800 border-t border-slate-100 dark:border-slate-700/50 flex flex-col sm:flex-row justify-end gap-3 px-5 pt-5 pb-5">
               ...
               <button onClick={handleApplyOptimization} className="px-6 py-2.5 bg-indigo-600 dark:bg-indigo-500 text-white font-bold rounded-xl hover:bg-indigo-700 transition-all shadow-lg shadow-indigo-500/20 dark:shadow-indigo-500/10 flex items-center gap-2 text-xs">
