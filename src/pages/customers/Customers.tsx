import { Users, Search, UserPlus, Mail, MapPin, ExternalLink, CheckCircle2, X, MessageSquare, Plus, Trash2 } from "../../components/Icons";
...
export function Customers() {
    ...
    const filteredCustomers = customers.filter((c: any) => c.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
        c.email.toLowerCase().includes(searchTerm.toLowerCase()));
    const handleSendMessage = (e: React.FormEvent) => {
        e.preventDefault();
        if (!message.trim() || !activeMessenger)
            return;
    ...
    return (<div className="space-y-6 sm:space-y-8 animate-fade-in transition-all duration-300">
        ...
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <div className="relative inline-flex items-center">
              <RotatingText texts={["Database", "Directory", "Relations", "Growth"]} mainClassName="text-xl sm:text-2xl font-bold text-slate-500 dark:text-slate-400 relative z-10" staggerDuration={0.02} rotationInterval={3500} splitBy="characters"/>
            </div>
          </div>
          <p className="text-sm text-slate-500 dark:text-slate-400 font-medium">View and manage your customer relationship.</p>
        </div>
        <button onClick={() => setIsAddingCustomer(true)} className="w-full sm:w-auto flex items-center justify-center gap-2 px-6 py-2.5 bg-indigo-600 text-white font-semibold rounded-xl hover:bg-indigo-700 transition-colors shadow-lg shadow-indigo-500/20 dark:shadow-indigo-500/10 text-sm">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4 sm:gap-6 transition-all duration-300">
        {filteredCustomers.length > 0 ? (filteredCustomers.map((customer: any) => (<div key={customer.id} className="transition-all duration-300">
              <SpotlightCard className="p-0 overflow-hidden">
                <div className="p-6 z-10 relative pointer-events-auto">
                  <div className="flex justify-between items-start mb-6">
                    <div className="relative">
                      <img src={customer.avatar} alt={customer.name} className="w-16 h-16 rounded-2xl border-2 border-white dark:border-slate-700/50 shadow-md object-cover cursor-pointer hover:scale-105 transition-transform" onClick={() => setSelectedCustomer(customer)}/>
                    ...
                    <div className="flex items-center gap-2">
                      <button onClick={() => { setActiveMessenger(customer); }} className="p-2 bg-indigo-50 dark:bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 rounded-xl hover:bg-indigo-100 transition-colors">
                  ...
                  <div className="mb-6">
                    <h3 onClick={() => setSelectedCustomer(customer)} className="text-lg font-bold text-slate-900 dark:text-slate-100 hover:text-indigo-600 transition-colors cursor-pointer">
                      {customer.name}
                    </h3>
            ...
            </div>))) : (<div className="col-span-full py-20 text-center transition-all duration-300">
            <div className="flex flex-col items-center gap-6">
              <div className="w-16 h-16 bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800/50 rounded-[2rem] flex items-center justify-center shadow-inner">
                {searchTerm ? (<Search className="w-7 h-7 text-slate-300 dark:text-slate-600"/>) : (<Users className="w-7 h-7 text-slate-300 dark:text-slate-600"/>)}
              </div>
              ...
              {searchTerm ? (<button onClick={() => setSearchTerm("")} className="mt-2 px-6 py-2.5 bg-slate-900 dark:bg-slate-800 text-white text-xs font-bold uppercase tracking-widest rounded-xl hover:bg-black dark:hover:bg-slate-700 transition-all border border-slate-800 dark:border-slate-700 shadow-lg">
                  Reset Search
                </button>) : (<button onClick={() => setIsAddingCustomer(true)} className="mt-2 px-6 py-2.5 bg-indigo-600 text-white text-xs font-bold uppercase tracking-widest rounded-xl hover:bg-indigo-700 transition-all shadow-lg shadow-indigo-500/20">
                  Add First Customer
                </button>)}
            </div>
          </div>)}
      ...
      {activeMessenger && (<>
          <div onClick={() => setActiveMessenger(null)} className="fixed inset-0 z-[1100] transition-opacity duration-300 bg-black/20"/>
          <div className="fixed inset-0 z-[1200] pointer-events-none transition-all duration-300">
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6" ref={scrollRef}>
              {currentMessages.length === 0 && (<div className="h-full flex flex-col items-center justify-center text-center opacity-40">
              ...
              {currentMessages.map((msg: any) => (<div key={msg.id} className={`flex transition-all duration-300 ${msg.sender === "me" ? "justify-end" : "justify-start"}`}>
            ...
            <form onSubmit={handleSendMessage} className="p-4 sm:p-6 bg-white dark:bg-slate-900 border-t border-slate-100 dark:border-slate-700/50">
              <div className="flex gap-2 p-1.5 sm:p-2 bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-xl sm:rounded-2xl items-center shadow-sm">
                <input value={message} onChange={(e) => setMessage(e.target.value)} placeholder="Type a message..." className="flex-1 bg-transparent border-none text-sm px-3 focus:ring-0 outline-none"/>
                <button type="submit" className="p-3 bg-indigo-600 text-white rounded-xl hover:bg-indigo-700 transition-colors shadow-lg shadow-indigo-500/20 dark:shadow-indigo-500/10">
      ...
      {isAddingCustomer && (<>
          <div onClick={() => setIsAddingCustomer(false)} className="fixed inset-0 z-[1100] transition-opacity duration-300 bg-black/20"/>
            ...
            <div className="relative w-full max-w-md bg-white dark:bg-slate-800 rounded-2xl sm:rounded-3xl shadow-2xl p-6 sm:p-8 pointer-events-auto">
             ...
             <form onSubmit={handleAddCustomer} className="space-y-4">
               <div className="flex justify-center mb-2">
                 <div onClick={() => customerFileRef.current?.click()} className="w-20 h-20 rounded-2xl border-2 border-dashed border-slate-300 dark:border-slate-600 bg-slate-50 dark:bg-slate-800/50 flex items-center justify-center cursor-pointer hover:border-indigo-400 dark:hover:border-indigo-400 hover:bg-indigo-50 dark:hover:bg-indigo-500/10 transition-all overflow-hidden">
                   {customerAvatarPreview ? (<img src={customerAvatarPreview} className="w-full h-full object-cover"/>) : (<Plus className="w-6 h-6 text-slate-400 dark:text-slate-500"/>)}
                 </div>
      ...
      {selectedCustomer && (<>
          <div onClick={() => setSelectedCustomer(null)} className="fixed inset-0 z-[1100] transition-opacity duration-300 bg-black/20"/>
          <div className="fixed inset-0 z-[1200] flex items-center justify-center p-4 pointer-events-none transition-all duration-300">
            ...
            <div className="h-32 bg-gradient-to-r from-indigo-500 to-violet-600 relative overflow-hidden">
               ...
               <button onClick={() => setSelectedCustomer(null)} className="absolute top-4 right-4 p-2 bg-white/20 hover:bg-white/30 text-white rounded-full transition-colors backdrop-blur-md">
                <X className="w-5 h-5"/>
              </button>
            </div>
            ...
            <div className="px-8 pb-8 relative">
              <div className="relative -mt-12 mb-6">
                <img src={selectedCustomer.avatar} alt={selectedCustomer.name} className="w-24 h-24 rounded-3xl border-4 border-white dark:border-slate-700/50 shadow-xl object-cover bg-white dark:bg-slate-800"/>
              ...
              <div className="flex gap-3">
                <button onClick={() => { setActiveMessenger(selectedCustomer); setSelectedCustomer(null); }} className="flex-1 py-3 bg-indigo-600 text-white font-bold rounded-xl hover:bg-indigo-700 transition-colors shadow-lg shadow-indigo-500/20 dark:shadow-indigo-500/10">
                  Send Message
                </button>
                <button onClick={() => { setSelectedCustomer(null); setIsAddingCustomer(true); }} className="px-6 py-3 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 font-bold rounded-xl hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors shadow-sm text-sm">
                  Edit
                </button>
                <button onClick={() => { setCustomers(customers.filter((c: any) => c.id !== selectedCustomer.id)); setSelectedCustomer(null); }} className="p-3 bg-rose-50 dark:bg-rose-500/10 text-rose-500 dark:text-rose-400 font-bold rounded-xl hover:bg-rose-100 transition-colors" title="Delete customer">
