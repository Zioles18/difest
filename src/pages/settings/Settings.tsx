import { User, Lock, Bell, CreditCard, ChevronRight, Shield, Save, Mail, Smartphone, Globe, Trash2, CheckCircle2, Sparkles, Zap, Loader2, X, CreditCard as PaymentIcon, TrendingUp, AlertTriangle, Monitor, LogOut, BarChart as BarChartIcon } from "../../components/Icons";
import { useState, useEffect } from "react";
import { auth } from "../../utils/auth";
import { useNavigate } from "../../lib/router";
import { SpotlightCard } from "../../components/ui/SpotlightCard";
import { getBusinessData, updateBusinessData, syncChartFromOrders, BUSINESS_DATA_UPDATED, BusinessData, addNotification } from "../../utils/store";
export function Settings() {
    ...
    return (<div className="max-w-5xl mx-auto space-y-6 sm:space-y-8 pb-20 animate-fade-in transition-all duration-300">
      ...
      <div className="flex flex-col lg:flex-row gap-6 sm:gap-8">
        ...
        <div className="flex lg:flex-col lg:w-64 shrink-0 gap-1 overflow-x-auto lg:overflow-visible pb-2 lg:pb-0 px-1 sm:px-0 no-scrollbar">
          {tabs.map((tab) => {
            const Icon = tab.icon;
            return (<button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`flex-shrink-0 lg:w-full flex items-center gap-2.5 sm:gap-3 px-4 sm:px-5 py-2.5 sm:py-3 rounded-xl text-xs sm:text-sm font-bold transition-all whitespace-nowrap ${activeTab === tab.id
        ...
        <div className="flex-1 px-1 sm:px-0">
          <SpotlightCard className="p-5 sm:p-8">
            <div className="relative">
              {activeTab === "general" && (<div className="space-y-8 z-10 relative animate-fade-in transition-all duration-300">
                  <div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div className="space-y-2">
                        ...
                        <input type="text" value={settings.firstName} onChange={(e) => setSettings({ ...settings, firstName: e.target.value })} className="w-full bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-3 text-sm focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-400 outline-none transition-all text-slate-900 dark:text-slate-100"/>
                      </div>
                      <div className="space-y-2">
                        <label className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest pl-1">Last Name</label>
                        <input type="text" value={settings.lastName} onChange={(e) => setSettings({ ...settings, lastName: e.target.value })} className="w-full bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-3 text-sm focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-400 outline-none transition-all text-slate-900 dark:text-slate-100"/>
                      </div>
                      <div className="md:col-span-2 space-y-2">
                        ...
                        <div className="relative">
                          ...
                          <input type="email" value={settings.email} onChange={(e) => setSettings({ ...settings, email: e.target.value })} className="w-full bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 rounded-xl py-3 pl-11 pr-4 text-sm focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-400 outline-none transition-all text-slate-900 dark:text-slate-100"/>
                        </div>
                      </div>
                    </div>
                  </div>
              ...
              {activeTab === "billing" && (<div className="space-y-8 z-10 relative animate-fade-in transition-all duration-300">
                  <div className="p-5 sm:p-6 bg-indigo-50 dark:bg-indigo-500/10 rounded-2xl border border-indigo-100 dark:border-indigo-800/50 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                    ...
                    {settings.plan !== "Business Premium" && (<button onClick={handleUpgrade} className="w-full sm:w-auto px-6 py-2.5 bg-indigo-600 text-white font-bold rounded-xl hover:bg-indigo-700 transition-all shadow-lg shadow-indigo-500/20 dark:shadow-indigo-500/10 text-sm flex items-center justify-center gap-2">
                  ...
                  <div>
                     ...
                     <div className="space-y-3">
                        {invoices.map((inv: any) => (<div key={inv.id} className="flex flex-col sm:flex-row items-start sm:items-center justify-between p-4 bg-slate-50 dark:bg-slate-900/50 rounded-xl border border-slate-100 dark:border-slate-700/50 group hover:border-indigo-100 transition-colors gap-4">
              ...
              {activeTab === "notifications" && (<div className="space-y-8 z-10 relative animate-fade-in transition-all duration-300">
                   <div>
                ...
                return (<div key={pref.id} className="flex flex-col sm:flex-row items-start sm:items-center justify-between p-5 bg-slate-50 dark:bg-slate-900/50 rounded-2xl border border-slate-100 dark:border-slate-700/50 gap-4">
                             ...
                             <button onClick={() => setSettings({ ...settings, [pref.id]: !settings[pref.id as keyof typeof settings] })} className={`w-12 h-6 rounded-full relative transition-colors duration-200 self-end sm:self-center ${settings[pref.id as keyof typeof settings] ? "bg-indigo-600" : "bg-slate-300 dark:bg-slate-600"}`}>
                                <div className={`absolute top-1 w-4 h-4 bg-white rounded-full shadow-sm transition-all duration-200 ${settings[pref.id as keyof typeof settings] ? 'translate-x-[22px]' : 'translate-x-1'}`}/>
                             </button>
                          </div>);
            })}
                    </div>
              ...
              {activeTab === "data" && (<div className="space-y-8 z-10 relative animate-fade-in transition-all duration-300">
                  <div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6 mb-8">
                        <div className="space-y-2">
                           ...
                           <div className="relative">
                             ...
                             <input type="text" inputMode="numeric" value={businessData.revenueGoal} onChange={(e) => {
                ...
                setBusinessData({ ...businessData, revenueGoal: val ? Number(val) : 0 });
                        ...
                        <div className="space-y-2">
                           ...
                           <div className="relative">
                             ...
                             <input type="text" inputMode="decimal" value={businessData.conversion} onChange={(e) => {
                const val = e.target.value.replace(/[^0-9.]/g, '');
                ...
                setBusinessData({ ...businessData, conversion: sanitized ? Number(sanitized) : 0 });
                    ...
                    <div className="p-5 sm:p-6 bg-slate-50 dark:bg-slate-900/50 rounded-[1.5rem] sm:rounded-[2rem] border border-slate-100 dark:border-slate-700/50 mb-8">
                       ...
                       <button onClick={handleSyncFromOrders} className="w-full py-4 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 text-slate-900 dark:text-slate-100 font-bold rounded-2xl hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-all shadow-sm flex items-center justify-center gap-2 text-sm">
                    ...
                    <div className="space-y-4">
                       ...
                       <div className="grid grid-cols-4 sm:grid-cols-7 gap-3">
                          {businessData.chartDataPeriods.week.map((d, i) => (<div key={d.name} className="space-y-1.5">
                                <label className="text-[9px] font-bold text-slate-400 dark:text-slate-500 uppercase text-center block">{d.name}</label>
                                <div className="relative">
                                  <input type="text" inputMode="numeric" value={d.value} onChange={(e) => {
                    const val = e.target.value.replace(/[^0-9]/g, '');
              ...
              {activeTab === "security" && (<div className="space-y-8 z-10 relative animate-fade-in transition-all duration-300">
                   <div>
                    ...
                    <div className="p-5 sm:p-6 bg-slate-50 dark:bg-slate-900/50 rounded-2xl border border-slate-100 dark:border-slate-700/50 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
                       ...
                       <button onClick={() => setSettings({ ...settings, twoFactor: !settings.twoFactor })} className={`w-full sm:w-auto px-6 py-2 rounded-xl text-xs font-bold transition-all ${settings.twoFactor ? "bg-emerald-50 dark:bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-100 dark:border-emerald-800/50" : "bg-indigo-600 text-white shadow-lg shadow-indigo-100"}`}>
                    ...
                    <div className="mb-6">
                      ...
                      <div className="space-y-3">
                          {sessions.map(s => {
                const Icon = s.icon;
                return (<div key={s.id} className="flex flex-col sm:flex-row items-start sm:items-center justify-between p-4 bg-slate-50 dark:bg-slate-900/50 rounded-xl border border-slate-100 dark:border-slate-700/50 gap-4 transition-all duration-300">
                                <div className="flex items-center gap-3">
                                ...
                                {!s.current && (<button onClick={() => handleRemoveSession(s.id)} className="p-2 text-slate-400 dark:text-slate-500 hover:text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-500/10 rounded-lg transition-colors" title="Remove session">
                    ...
                    <button onClick={() => setShowDeleteConfirm(true)} className="text-xs font-bold text-rose-500 dark:text-rose-400 flex items-center gap-2 px-4 py-2 hover:bg-rose-50 dark:hover:bg-rose-500/10 rounded-lg transition-colors">
            ...
            <div className="mt-12 pt-8 border-t border-slate-100 dark:border-slate-700/50 flex flex-col sm:flex-row justify-end items-center gap-3 z-10 relative">
               ...
               <button onClick={activeTab === "data" ? handleSaveBusinessData : handleSave} disabled={isSaving} className="px-8 py-3 bg-indigo-600 text-white font-bold rounded-2xl hover:bg-indigo-700 transition-all shadow-xl shadow-indigo-500/30 dark:shadow-indigo-500/10 text-sm flex items-center gap-2 min-w-[140px] justify-center">
      ...
      {showToast && (<div className="fixed bottom-10 left-1/2 -translate-x-1/2 z-[100] bg-slate-900 dark:bg-slate-900 text-white px-8 py-4 rounded-2xl shadow-2xl flex items-center gap-3 border border-slate-800 dark:border-slate-700/50 animate-fade-in transition-all duration-300">
      ...
      {showDeleteConfirm && (<>
          <div onClick={() => { setShowDeleteConfirm(false); setDeleteConfirmText(""); }} className="fixed inset-0 z-[1100] bg-black/20 transition-opacity duration-300"/>
          <div className="fixed inset-0 z-[1200] flex items-center justify-center p-4 pointer-events-none transition-all duration-300">
            <div className="relative w-full max-w-md bg-white dark:bg-slate-900 rounded-[2.5rem] p-10 shadow-2xl pointer-events-auto">
              ...
              <div className="space-y-3 mb-6">
                ...
                <input type="text" value={deleteConfirmText} onChange={(e) => setDeleteConfirmText(e.target.value)} placeholder="DELETE" className="w-full bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-3 text-sm font-bold focus:ring-4 focus:ring-rose-500/10 focus:border-rose-400 outline-none transition-all text-center tracking-widest text-slate-900 dark:text-slate-100"/>
              </div>
              <div className="flex gap-3">
                <button onClick={() => { setShowDeleteConfirm(false); setDeleteConfirmText(""); }} className="flex-1 py-4 bg-slate-100 dark:bg-slate-900 text-slate-600 dark:text-slate-300 font-bold rounded-2xl hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors text-sm">
                  Cancel
                </button>
                <button onClick={handleDeleteAccount} disabled={deleteConfirmText !== "DELETE"} className="flex-1 py-4 bg-rose-600 text-white font-bold rounded-2xl hover:bg-rose-700 transition-all shadow-xl shadow-rose-500/20 dark:shadow-rose-500/10 text-sm disabled:opacity-40 disabled:cursor-not-allowed">
                  Delete Forever
                </button>
              </div>
            </div>
          ...
          <div className="fixed inset-0 z-[1200] flex items-center justify-center p-4 pointer-events-none transition-all duration-300">
            <div className="relative w-full max-w-md bg-white dark:bg-slate-900 rounded-[2.5rem] p-10 text-center shadow-2xl pointer-events-auto">
             {paymentStep === 1 ? (<>
                 ...
                 <button onClick={() => setIsPaying(false)} className="w-full py-4 bg-indigo-600 text-white font-bold rounded-2xl hover:bg-indigo-700 transition-all shadow-xl shadow-indigo-500/30 dark:shadow-indigo-500/10">
                   Get Started
                 </button>
               </>)}
            </div>
