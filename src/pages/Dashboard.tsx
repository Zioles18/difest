          ...
          <>
            ...
            <div className="fixed inset-0 z-[1200] flex items-center justify-center p-4 pointer-events-none">
              ...
              >
                ...
                {/* Period Tabs */}
                <div className="flex bg-slate-100/60 dark:bg-slate-800/60 p-1.5 rounded-2xl border border-slate-100 dark:border-slate-700/50 mb-5 sm:mb-8 gap-1">
                  {(["week", "month", "year"] as const).map(tab => (
                    <button
                      key={tab}
                      onClick={() => setEditorTab(tab)}
                      className={`flex-1 py-2.5 text-xs font-bold uppercase tracking-widest rounded-xl transition-all ${
                        tab === editorTab ? "bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100 shadow-sm" : "text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-300"
                      }`}
                    >
