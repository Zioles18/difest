import { TrendingUp, Users, ShoppingBag, Target, ArrowUpRight, ArrowDownRight, Calendar, Download, Eye, CheckCircle2, Zap, BarChart3, X, Activity, CheckSquare, Clock, Edit3, Save } from "../../components/Icons";
...
export function Dashboard() {
    ...
    const kpiChanges: Record<string, {
        change: string;
        trend: "up" | "down";
    }> = {
    ...
    return (<div className="space-y-4 sm:space-y-6 lg:space-y-10 animate-fade-in transition-all duration-300">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 sm:gap-6">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <div className="relative inline-flex items-center">
              <RotatingText texts={["Overview", "Intelligence", "Insights", "Metrics"]} mainClassName="text-2xl sm:text-3xl font-bold text-slate-500 dark:text-slate-400 relative z-10" staggerDuration={0.02} rotationInterval={3500} splitBy="characters"/>
            </div>
          </div>
        ...
        <div className="flex items-center gap-2 sm:gap-3 flex-wrap">
          ...
          <button onClick={openChartEditor} className="flex items-center gap-2 px-3 sm:px-4 py-2 sm:py-2.5 bg-indigo-50 dark:bg-indigo-500/10 text-indigo-700 dark:text-indigo-300 border border-indigo-100 dark:border-indigo-500/20 rounded-2xl shadow-sm hover:bg-indigo-100 dark:hover:bg-indigo-500/20 transition-all text-xs sm:text-sm font-bold">
            <Edit3 className="w-4 h-4"/>
            <span className="hidden sm:inline">Edit Chart Data</span>
          </button>
          <button onClick={handlePdfDownload} disabled={isPdfLoading} className="p-2.5 sm:p-3 bg-slate-900 text-white rounded-2xl shadow-xl shadow-slate-200 dark:shadow-slate-900/50 hover:bg-black transition-all disabled:opacity-60 flex items-center gap-2 text-sm font-bold">
            {isPdfLoading ? (<svg className="animate-spin h-5 w-5" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"/><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"/></svg>) : (<Download className="w-5 h-5"/>)}
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 lg:gap-8 transition-all duration-300">
        {kpis.map((kpi, idx) => (<div key={idx} onClick={() => setSelectedKPI(kpi.id)} className="cursor-pointer group transition-all duration-300">
        ...
        <div className="lg:col-span-2 transition-all duration-300">
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6 sm:mb-10 relative z-10">
            ...
            <div className="flex bg-slate-100/80 dark:bg-slate-800/50 p-1 rounded-xl border border-slate-200/70 dark:border-slate-700/50 self-start sm:self-auto">
               {(["Week", "Month", "Year"] as const).map(t => (<button key={t} onClick={() => setChartRange(t)} className={`px-5 py-2 text-[10px] font-bold uppercase tracking-widest rounded-lg transition-all ${t === chartRange ? "bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100 shadow-sm" : "text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-300"}`}>{t}</button>))}
            </div>
          </div>
        ...
        <div className="lg:col-span-1 transition-all duration-300">
          <SpotlightCard className="p-5 sm:p-8 lg:p-10 bg-white/80 dark:bg-slate-800/40 border border-slate-200/70 dark:border-slate-700/50 relative overflow-hidden">
           ...
           <div className="space-y-4 sm:space-y-6 lg:space-y-8 relative z-10">
              {recentOrders.length > 0 ? recentOrders.map((act, i) => (<div key={i} className="flex items-center justify-between group cursor-pointer p-1">
                ...
                </div>)) : (<div className="text-center py-10 opacity-40">
                   <p className="text-sm font-bold text-slate-600 dark:text-slate-300">Waiting for new data...</p>
                </div>)}
           </div>
           
           <button onClick={() => navigate("/analytics")} className="w-full mt-10 py-4 bg-slate-900 dark:bg-slate-800 text-white text-xs font-bold uppercase tracking-widest rounded-2xl transition-all flex items-center justify-center gap-3 shadow-xl shadow-slate-900/40 dark:shadow-black/60 hover:bg-black relative z-10">
      ...
      {chartEditorOpen && (<>
          <div onClick={() => setChartEditorOpen(false)} className="fixed inset-0 z-[1100] transition-opacity duration-300"/>
          <div className="fixed inset-0 z-[1200] flex items-center justify-center p-4 pointer-events-none transition-all duration-300">
            <div className="relative w-full max-w-2xl bg-white dark:bg-slate-800 rounded-[1.5rem] sm:rounded-[2.5rem] p-5 sm:p-8 lg:p-10 shadow-2xl border border-slate-100 dark:border-slate-700/50 pointer-events-auto max-h-[85vh] flex flex-col">
              ...
              <div className="flex bg-slate-100/60 dark:bg-white/5 p-1.5 rounded-2xl border border-slate-100 dark:border-white/10 backdrop-blur-md mb-5 sm:mb-8 gap-1">
                {(["week", "month", "year"] as const).map(tab => (<button key={tab} onClick={() => setEditorTab(tab)} className={`flex-1 py-2.5 text-xs font-bold uppercase tracking-widest rounded-xl transition-all ${tab === editorTab
              ...
              <div className="overflow-y-auto flex-1 pr-1">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                  {draftData[editorTab].map((point, idx) => (<NumberInput key={`${editorTab}-${idx}`} label={point.name} name={`value-${idx}`} value={point.value} onChange={(val) => updateDraftValue(editorTab, idx, val)} step={1} className="p-4 bg-slate-50 dark:bg-slate-800/50 rounded-3xl border border-slate-100 dark:border-slate-700/50 !space-y-2" showMultiplier={true}/>))}
                </div>
              </div>

              
              <div className="flex gap-3 sm:gap-4 mt-5 sm:mt-8 pt-4 sm:pt-6 border-t border-slate-100 dark:border-slate-700/50">
                <button onClick={() => setChartEditorOpen(false)} className="flex-1 py-4 bg-slate-100 dark:bg-white/5 text-slate-600 dark:text-slate-300 border border-slate-200 dark:border-white/10 dark:border-t-white/20 backdrop-blur-md font-bold rounded-2xl hover:bg-slate-200 dark:hover:bg-white/10 transition-all text-sm shadow-sm dark:shadow-xl dark:shadow-black/20" disabled={isSaving}>
                  Cancel
                </button>
                <button onClick={saveChartData} disabled={isSaving} className="flex-1 py-4 bg-indigo-600 text-white font-bold rounded-2xl hover:bg-indigo-700 transition-all shadow-xl shadow-indigo-500/20 dark:shadow-indigo-500/10 text-sm flex items-center justify-center gap-2">
                  {isSaving ? (<>
      ...
      {selectedKPI && (<>
          <div onClick={() => setSelectedKPI(null)} className="fixed inset-0 z-[1100] transition-opacity duration-300"/>
          <div className="fixed inset-0 z-[1200] flex items-center justify-center p-4 pointer-events-none transition-all duration-300">
            <div className="relative w-full max-w-lg bg-white dark:bg-slate-800 rounded-[1.5rem] sm:rounded-[3rem] p-6 sm:p-10 lg:p-12 shadow-2xl border border-slate-100 dark:border-slate-700/50 pointer-events-auto max-h-[90vh] overflow-y-auto">
             <div className="flex items-center justify-between mb-6 sm:mb-10">
                <div className="flex items-center gap-5">
                   <div className={`w-16 h-16 rounded-[1.5rem] flex items-center justify-center text-white shadow-2xl ${selectedKPI === 'revenue' ? 'bg-indigo-600' :
      ...
      {showPdfToast && (<div className="fixed bottom-10 left-1/2 -translate-x-1/2 z-[300] bg-slate-900 dark:bg-slate-800 text-white px-8 py-4 rounded-2xl shadow-2xl flex items-center gap-3 border border-slate-800 dark:border-slate-700 transition-all duration-300">
